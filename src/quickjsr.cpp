...
extern "C" {
  SEXP qjs_context_(SEXP stack_size_) {
    BEGIN_CPP11
    const int stack_size = INTEGER_ELT(stack_size_, 0);
  ...
  SEXP qjs_source_(SEXP ctx_ptr_, SEXP input_, SEXP is_file_) {
    ...
    int ret;
    const bool is_file = LOGICAL_ELT(is_file_, 0);
  ...
  SEXP to_json_(SEXP arg_, SEXP auto_unbox_) {
    ...
    JSRuntime* rt = quickjsr::JS_NewCustomRuntime(0);
    JSContext* ctx = quickjsr::JS_NewCustomContext(rt);
    const bool auto_unbox = LOGICAL_ELT(auto_unbox_, 0);

    JSValue arg = quickjsr::SEXP_to_JSValue(ctx, arg_, auto_unbox);
  ...
  SEXP from_json_(SEXP json_) {
    ...
    std::string json = Rf_translateCharUTF8(STRING_ELT(json_, 0));
