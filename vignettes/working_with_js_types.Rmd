...
Note that the `to_json()` function operates by converting R objects to their JS equivalents, and then calling `JSON.stringify()` on the result. This allows you explore how different types are being converted to JS.
...
Values in the environment can also updated from JS code:
...
```{r}
ctx$source(code = 'function getwd() { return R.package("base").getwd() }')
ctx$call("getwd")
```